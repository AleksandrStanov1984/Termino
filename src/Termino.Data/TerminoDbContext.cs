...
public class TerminoDbContext : DbContext
{
    ...
    public TerminoDbContext( DbContextOptions<TerminoDbContext> options ) : base( options )
    {
    ...
    protected override void OnModelCreating( ModelBuilder b )
    {
        ...
        var dtoN = new ValueConverter<DateTimeOffset?,long?>
        (
            v => v.HasValue? v.Value.ToUnixTimeSeconds(): (long?)null,
            v => v.HasValue? DateTimeOffset.FromUnixTimeSeconds( v.Value ): ( DateTimeOffset? )null
        );
